/*
  ...
  int vsc824x_config_init(struct phy_device *phydev)
 {
         int err;
...
static struct of_device_id vitesse_match[] = {
	{
...
MODULE_DEVICE_TABLE(of, vitesse_match);
 ...
 static struct phy_driver vsc82xx_driver[] = {
 {
         ...
         .driver		= {
 		.owner = THIS_MODULE,
 		.of_match_table = &vitesse_match[1],
 	},
 }, 
 {
		  ...
		  .driver		= {
 		.owner = THIS_MODULE,
 		.of_match_table = &vitesse_match[0],
 	},
  }, {
 ...
 static int __init vitesse_init(void)
{
	int ret;

	ret = phy_driver_register(&vsc82xx_driver[2]);
	if (ret)
		goto err1;

	ret = phy_driver_register(&vsc82xx_driver[3]);
	if (ret)
		goto err2;
...
err2:
	phy_driver_unregister(&vsc82xx_driver[2]);
err1:
	return ret;
}

static void __exit vitesse_exit(void)
{
	phy_driver_unregister(&vsc82xx_driver[2]);
	phy_driver_unregister(&vsc82xx_driver[3]);
	
}

module_init(vitesse_init);
module_exit(vitesse_exit);
